...
# 超参数
batch_size = 64 # 一次训练多少句话
block_size = 256 # 一句话的字符串长度

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

n_embedding = 384 # 嵌入后的维度 尽量是 2的次方数

learning_rate = 3e-4 # 学习率

max_iters = 100000 # 训练次数

dropout_value = 0.2 # 每次运行有20%的节点归零，不参与运算
...
class LanguageModel(nn.Module):
    def __init__(self):
        ...
        self.head = Head(head_size=n_embedding)
    ...
    def forward(self, idx, targets=None):
        ...
        logits = torch.relu(self.fc1(head_out)) # 输入的嵌入 经过全连接层 维度(B, T, vocab_size)
