...
# 超参数
batch_size = 64# 一次训练多少句话 训练时选择合适大小 保证内存资源占用
block_size = 256 # 一句话的字符串长度 训练时太长的序列可能导致内存问题

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

n_embedding = 384 # 词嵌入后的维度 尽量是 2的次方数 嵌入维度尽量大以提高模型容量

num_heads = 6 # 注意力头的数量

head_size = n_embedding // num_heads # 每个注意力头的维度 如query、key、value的维度

n_layers = 6 # 残差网络的层数 包含：多头注意力、前馈网络、归一化层 残差

learning_rate = 3e-4 # 学习率 保持稳定性

max_iters = 20000  # 训练轮次

dropout_value = 0.2 # 每次运行有10%的节点归零，不参与运算，减少数据干扰并保持更多信息 防止过拟合
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.self_attention(self.ln1(x)) # 残差多头注意力网络
        x = x + self.feed_forward(self.ln2(x)) # 残差线性前馈网络层
        return x
...
class LanguageModel(nn.Module):
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embedding, num_heads) for _ in range(n_layers)]) # 多级残差多头注意力
    ...
    '''生成文本'''
    def generate(self, token_sequ, max_new_tokens, temperature=1.0):
        for _ in range(max_new_tokens):
            ...
            '''
            logits = logits[:, -1, :] / temperature # 只取序列最后一个字符的logits(概率分布的向量) 并增加温度缩放
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_iters) # 学习率调度器
    ...
    for i in range(max_iters):
        ...
        # 保存模型类
        def save_model(model, optimizer, loss, epoch, filename):
            torch.save({
        ...
        # 保存模型
        save_model(model, optimizer, losses['val'], i, 'transformer_model.pth')
